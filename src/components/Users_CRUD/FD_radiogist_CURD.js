...
class FD_radiogist extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = { 
      ...
      phone: ""  
        ...
        getTypeByID = async(id) => {
          ...
          let response = await fetch(`http://localhost:2400/fd_radiogist/${id}`);
        ...
        getFD_radiogistTypesList = (fd_chemist) =>{
        for(var type in fd_radiogistList){
    ...
    getData = async()=>{
      await axios.get(' http://localhost:2400/fd_radiogist').then(async resp => {
        // return resp.data;
         this.setState({
            fd_radiogistList : resp.data
        })
      })
    }
    ...
    refreshAfterDeletion = (id)=>{
     this.setState({
        fd_chemistList: this.state.fd_radiogistList.filter(row => row.id !== id)
     })
    }
 
    handleDelete= async(id)=>{
        await axios.delete(`http://localhost:2400/fd_radiogist/${id}`)
        .then(res => {
          console.log(res);
   ...
   async componentDidMount(){
    ...
    handleUpdate = ()=>{
      ...
      axios.put(`http://localhost:2400/fd_radiogist/${id}` , obj)
         .then(res => {
           console.log(res);
           console.log(res.data);
         })
    ...
    handleAdding = () =>{
      ...
      console.log("type: ", obj);
      axios.post(`http://localhost:2400/fd_radiogist`,  obj )
      .then(res => {
        console.log(res);
        console.log(res.data);
      })
...
export default withStyles(useStyles)(FD_radiogist); 
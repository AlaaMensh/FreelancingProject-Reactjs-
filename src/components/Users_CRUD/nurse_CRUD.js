...
class Nurse extends Component {
  constructor(props) {
    ...
    this.state = { 
      ...
      phone: ""  
        ...
        getTypeByID = async(id) => {
          ...
          let response = await fetch(`http://localhost:2400/nurse/${id}`);
        ...
        getNurseTypesList = (nurseList) =>{
        for(var type in nurseList){
    ...
    getData = async()=>{
      await axios.get(' http://localhost:2400/nurse').then(async resp => {
        // return resp.data;
         this.setState({
    ...
    refreshAfterDeletion = (id)=>{
     this.setState({
      nurseList: this.state.nurseList.filter(row => row.id !== id)
     })
    }
 
    handleDelete= async(id)=>{
        await axios.delete(`http://localhost:2400/nurse/${id}`)
        .then(res => {
          console.log(res);
   ...
   async componentDidMount(){
    ...
    handleUpdate = ()=>{
      ...
      axios.put(`http://localhost:2400/nurse/${id}` , obj)
         .then(res => {
           console.log(res);
           console.log(res.data);
         })
...
export default withStyles(useStyles)(Nurse); 
...
class DoctorFD extends Component {
  constructor(props) {
    ...
    this.state = { 
      ...
      phone: ""  
        ...
        getTypeByID = async(id) => {
          ...
          let response = await fetch(`http://localhost:2400/fd_doctors/${id}`);
        ...
        getFD_doctorsTypesList = (fd_doctors) =>{
        for(var type in fd_doctorsList){
    ...
    getData = async()=>{
      await axios.get(' http://localhost:2400/fd_doctors').then(async resp => {
        // return resp.data;
         this.setState({
            fd_doctorsList : resp.data
        })
      })
    }
    ...
    refreshAfterDeletion = (id)=>{
     this.setState({
        fd_doctorsList: this.state.fd_doctorsList.filter(row => row.id !== id)
     })
    }
 
    handleDelete= async(id)=>{
        await axios.delete(`http://localhost:2400/fd_doctors/${id}`)
        .then(res => {
          console.log(res);
   ...
   async componentDidMount(){
    ...
    handleUpdate = ()=>{
      ...
      axios.put(`http://localhost:2400/fd_doctors/${id}` , obj)
         .then(res => {
           console.log(res);
           console.log(res.data);
         })
    ...
    handleAdding = () =>{
      ...
      console.log("type: ", obj);
      axios.post(`http://localhost:2400/fd_doctors`,  obj )
      .then(res => {
        console.log(res);
        console.log(res.data);
      })

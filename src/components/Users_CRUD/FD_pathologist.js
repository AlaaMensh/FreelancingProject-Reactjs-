...
class FD_pathologist extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = { 
      ...
      phone: ""  
        ...
        getTypeByID = async(id) => {
          ...
          let response = await fetch(`http://localhost:2400/fd_pathologist/${id}`);
        ...
        getFD_pathologistTypesList = (fd_pathologist) =>{
    ...
    getData = async()=>{
      await axios.get(' http://localhost:2400/fd_pathologist').then(async resp => {
        // return resp.data;
         this.setState({
            fd_pathologistList : resp.data
        })
      })
    }
    ...
    refreshAfterDeletion = (id)=>{
     this.setState({
        fd_pathologistList: this.state.fd_pathologistList.filter(row => row.id !== id)
     })
    }
 
    handleDelete= async(id)=>{
        await axios.delete(`http://localhost:2400/fd_pathologist/${id}`)
        .then(res => {
          console.log(res);
   ...
   async componentDidMount(){
    ...
    handleUpdate = ()=>{
      ...
      axios.put(`http://localhost:2400/fd_pathologist/${id}` , obj)
         .then(res => {
           console.log(res);
           console.log(res.data);
         })
    ...
    handleAdding = () =>{
      ...
      console.log("type: ", obj);
      axios.post(`http://localhost:2400/fd_pathologist`,  obj )
      .then(res => {
        console.log(res);
        console.log(res.data);
      })
...
export default withStyles(useStyles)(FD_pathologist); 
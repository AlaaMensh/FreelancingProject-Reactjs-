...
class FamilyHistoryStep extends Component {
  constructor(props) {
    ...
    handleDelete= async(id)=>{
      this.setState({
        familyHistoryList: this.state.familyHistoryList.filter(row => row.id !== id)
       }) 
    }
   async componentDidMount(){
     if(this.props.familyHistoryListHome.length > 0 && this.props.familyHistoryListHome.length ){
       this.setState({familyHistoryList : this.props.familyHistoryListHome , key:this.props.familyHistoryListHome.length+1})
     }
      // this.getData()
    ...
    handleUpdate = ()=>{
      ...
      if(!details.notes){
        ...
        const items = this.state.familyHistoryList.map(
        ...
        return(
          ...
          <div className="row">
            <div className="col-auto px-2 py-2 text-center rounded  header">
                <span className="">Family History</span>
            </div>
            ...
            <div className = "row gridDataHeader align-items-center" style={{ height: 400, width: '100%' }}>
               <DataGrid className="datagrid bg-light  rounded MuiDataGrid-cellCenter" style={{textAlign:"center"}} rows={this.state.familyHistoryList} columns={[
                 { field: 'id', headerName: 'id', width: 70 },
              { field: 'relation', headerName: 'Relation', width: 200 },
              { field: 'problem', headerName: 'Problem', width: 200 },
              { field: 'dateOfDiagnosis', headerName: 'dateOfDiagnosis', width: 200 },
